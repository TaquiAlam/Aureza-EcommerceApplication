...
export default function SellerLayout() {
  ...
  const navItems = [
    { name: 'Products Inventory', path: '/seller/products', icon: Package },
    { name: 'Customer Orders', path: '/seller/orders', icon: ShoppingCart },
  ];
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider bg-[#FF9900] text-[#131921] px-1.5 py-0.5 rounded font-black">
                Seller
              </span>
            </div>
            <p className="text-[11px] text-gray-400">Merchant Central</p>
          </div>
        </div>
      </div>

      {/* Merchant Notice Banner */}
      <div className="mx-3 mt-4 p-3 rounded-xl bg-[#232F3E]/80 border border-[#FF9900]/30 text-xs text-amber-200/90 flex items-start gap-2">
        <Sparkles size={16} className="text-[#FF9900] shrink-0 mt-0.5" />
        <p className="text-[11px] leading-snug">
          Manage your products inventory & dispatch customer orders seamlessly.
        </p>
      </div>

      {/* Navigation */}
      <div className="flex-1 overflow-y-auto py-4 px-3 flex flex-col gap-1.5">
        <p className="px-3 text-[10px] font-bold tracking-wider uppercase text-gray-400 mb-2">Seller Access</p>
        {navItems.map((item) => {
          const isActive = location.pathname === item.path || (item.path === '/seller/products' && location.pathname === '/seller');
          ...
          return (
            <Link
              ...
              className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
                isActive 
                  ? 'bg-[#FF9900] text-[#131921] shadow-md shadow-amber-500/20 font-bold' 
      ...
      <div className="p-4 border-t border-[#232F3E] bg-[#0c1117] flex flex-col gap-2">
        <div className="flex items-center gap-2.5 px-2 py-1">
          ...
          <div className="overflow-hidden">
            <p className="text-xs font-bold text-white truncate">{user?.username || 'Seller Partner'}</p>
            <p className="text-[10px] text-gray-400 flex items-center gap-1">
              <ShieldCheck size={11} className="text-emerald-400" /> Verified Merchant
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full mt-2 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 border border-rose-500/20 transition-colors cursor-pointer"
        >
          <LogOut size={14} />
          <span>Sign Out</span>
        </button>
      ...
      <main className="flex-1 lg:pl-64 flex flex-col min-h-screen w-full">
        ...
        <header className="bg-white border-b border-[#E8E2D6] px-4 sm:px-6 py-3.5 sticky top-0 z-20 flex items-center justify-between shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-gray-500 font-medium">Aureza Merchant Central</p>
              <h2 className="text-sm font-bold text-gray-900">Seller Portal</h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <ExternalLink size={14} className="text-[#FF9900]" />
